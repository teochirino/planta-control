    ...
    <div class="min-h-screen" style="background: #eaf0f5;">
        ...
        <div :class="isTVMode() ? 'p-8 ml-16 2xl:p-12 2xl:ml-20' : 'p-6 ml-16'">
            ...
            <div :class="isTVMode() ? 'p-8 2xl:p-12' : 'p-6'" class="bg-white border border-[#d4dee8] rounded-xl shadow-sm">
                ...
                <div v-if="dailyPrograms.length > 0" :class="isTVMode() ? 'mb-8' : 'mb-6'">
                    <h2 :class="isTVMode() ? 'text-2xl mb-6' : 'text-xl mb-4'" class="font-semibold text-[#0b2a40]">Programas Diarios - {{ selectedPhase ? getPhaseLabel(selectedPhase) : '' }}</h2>
                    <div :class="isTVMode() ? 'space-y-6' : 'space-y-4'">
                        <div v-for="dailyProgram in dailyPrograms" :key="dailyProgram.id" 
                            ...
                            <div :class="isTVMode() ? 'gap-6 text-base' : 'gap-4 text-sm'" class="grid grid-cols-2 md:grid-cols-5">
                                <div>
                ...
                <div v-else-if="selectedProgramId && selectedWorkCenterId && selectedPhase" :class="isTVMode() ? 'py-12' : 'py-8'" class="text-center">
                    <p :class="isTVMode() ? 'text-lg' : ''" class="text-[#6a8090] font-semibold">No hay programas diarios para la fecha y centro seleccionados</p>
                </div>

                <!-- Modal de Edición -->
                <div v-if="editingProgram" class="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
                    <div :class="isTVMode() ? 'p-8 max-w-4xl' : 'p-6 max-w-2xl'" class="rounded-lg mx-4 bg-white border border-[#d4dee8] shadow-2xl">
                        <h2 :class="isTVMode() ? 'text-2xl mb-6' : 'text-xl mb-4'" class="font-semibold text-[#0b2a40]">Editar Programa Diario</h2>
                        
                        <div :class="isTVMode() ? 'mb-6' : 'mb-4'">
                            <p :class="isTVMode() ? 'text-base' : 'text-sm'" class="font-semibold text-[#6a8090]">
                        ...
                        <form @submit.prevent="saveAdjustment">
                            <div :class="isTVMode() ? 'gap-6 mb-6' : 'gap-4 mb-4'" class="grid grid-cols-1 md:grid-cols-3">
                                <div>
                            ...
                            <div :class="isTVMode() ? 'gap-6 mb-6' : 'gap-4 mb-4'" class="grid grid-cols-1 md:grid-cols-2">
                                <div>
                            ...
                            <div :class="isTVMode() ? 'mb-6' : 'mb-4'">
                                <label :class="isTVMode() ? 'text-base' : 'text-sm'" class="block font-semibold mb-2 text-[#4e6070]">Motivo del ajuste *</label>
                                <textarea v-model="editForm.reason" :class="isTVMode() ? 'rows-4 px-5 py-3 text-base' : 'rows-3 px-4 py-2 text-sm'" required
                                          class="w-full rounded-lg font-semibold focus:outline-none bg-white text-[#0c1c28] border border-[#d4dee8]"
                                          placeholder="Explique el motivo del ajuste..."></textarea>
                            </div>

                            <div :class="isTVMode() ? 'mb-6' : 'mb-4'">
                                <label :class="isTVMode() ? 'text-base' : 'text-sm'" class="block font-semibold mb-2 text-[#4e6070]">Notas adicionales</label>
                                <textarea v-model="editForm.notes" :class="isTVMode() ? 'rows-3 px-5 py-3 text-base' : 'rows-2 px-4 py-2 text-sm'"
                                          class="w-full rounded-lg font-semibold focus:outline-none bg-white text-[#0c1c28] border border-[#d4dee8]"
                                          placeholder="Información adicional opcional..."></textarea>
                            </div>

                            <div :class="isTVMode() ? 'space-x-6' : 'space-x-4'" class="flex">
                                <button type="submit" 
                                        :disabled="editForm.processing"
                                        :class="isTVMode() ? 'px-8 py-3 text-base' : 'px-6 py-2 text-sm'"
                                        class="rounded-lg transition font-semibold disabled:opacity-50 bg-[#0a7c3e] text-white">
                                    {{ editForm.processing ? 'Guardando...' : 'Guardar Cambios' }}
                                </button>
                                <button type="button" @click="closeEdit"
                                        :class="isTVMode() ? 'px-8 py-3 text-base' : 'px-6 py-2 text-sm'"
                                        class="rounded-lg transition font-semibold bg-white text-[#0b2a40] border border-[#d4dee8]">
                                    Cancelar
                                </button>
                            </div>
                        </form>
