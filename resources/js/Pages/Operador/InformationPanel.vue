<template>
    <div class="min-h-screen" style="background: #eaf0f5;">
        ...
        <div :class="isTVMode() ? 'p-8' : 'p-6'">
            ...
            <div v-if="selectedWorkCenterData" :class="isTVMode() ? 'p-8' : 'p-6'" class="bg-white border border-[#d4dee8] rounded-xl shadow-sm">
                <h2 :class="isTVMode() ? 'text-3xl' : 'text-xl'" class="font-extrabold text-[#0b2a40] mb-4">{{ selectedWorkCenterData.name }}</h2>
                
                <!-- Tarjetas de KPIs del Centro (igual que supervisor) -->
                <div v-if="centerKPIsData" :class="isTVMode() ? 'mb-8 p-6' : 'mb-6 p-4'" class="bg-[#f8f9fb] border border-[#d4dee8] rounded-lg">
