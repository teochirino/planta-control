    ...
    <div class="min-h-screen" style="background: #eaf0f5;">
        ...
        <div class="p-6 ml-16">
            <div class="mb-6 flex justify-between items-center">
                <div>
                    <h1 class="text-3xl font-bold" style="color: #0b2a40;">Historial de Ajustes de Balance</h1>
                    <p class="text-sm mt-2" style="color: #6a8090;">Registro de todos los cambios en balances iniciales</p>
                </div>
                <button @click="goBack"
                    ...
                    <tbody>
                        <tr v-for="adjustment in adjustments.data" :key="adjustment.id" style="border-bottom: 1px solid #e8eff4;">
                            ...
                            <td class="px-4 py-3 text-sm">
                                <span :class="getFieldClass(adjustment.field_adjusted)"
                                      class="px-2 py-1 rounded text-xs font-semibold">
                                    {{ adjustment.field_label }}
                                </span>
                            </td>
...
function applyFilters() {
    router.get(route('ingeniero-procesos.work-center-balances.history'), filters.value, {
        preserveState: true,
    });
}
...
function getFieldClass(field) {
    switch (field) {
        case 'accumulated_backwardness': return 'bg-yellow-600';
        case 'accumulated_advanced': return 'bg-green-600';
