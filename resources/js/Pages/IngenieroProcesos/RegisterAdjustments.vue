<template>
    <div class="min-h-screen" style="background: #eaf0f5;">
        ...
        <div class="p-6 ml-16">
            <div class="mb-6">
                <h1 class="text-3xl font-bold" style="color: #0b2a40;">Registrar Ajustes de Producción</h1>
            </div>
            ...
            <div class="rounded-lg p-6" style="background: #fff; border: 1px solid #d4dee8; box-shadow: 0 2px 12px rgba(11,28,40,.08);">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    <div>
                        <label class="block text-sm font-semibold mb-2" style="color: #4e6070;">Programa *</label>
                    ...
                    <div>
                        <label class="block text-sm font-semibold mb-2" style="color: #4e6070;">Centro de Trabajo *</label>
                    ...
                    <div>
                        <label class="block text-sm font-semibold mb-2" style="color: #4e6070;">Fase *</label>
                        <select v-model="selectedPhase" @change="loadDailyPrograms"
                                :disabled="!selectedProgramId"
                                class="w-full px-4 py-2 rounded-lg font-semibold focus:outline-none disabled:opacity-50"
                                style="background: #fff; color: #0c1c28; border: 1px solid #d4dee8;">
                ...
                <div v-if="dailyPrograms.length > 0" class="mb-6">
                    <h2 class="text-xl font-semibold mb-4" style="color: #0b2a40;">Programas Diarios - {{ selectedPhase ? getPhaseLabel(selectedPhase) : '' }}</h2>
                    <div class="space-y-4">
                        <div v-for="dailyProgram in dailyPrograms" :key="dailyProgram.id" 
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-5 gap-4 text-sm">
                                <div>
                                    <span style="color: #6a8090;">Programado:</span>
                                    <span class="ml-2" style="color: #0c1c28; font-weight: 600;">{{ dailyProgram.programmed }}</span>
                                </div>
                                <div>
                                    <span style="color: #6a8090;">Atrasos:</span>
                                    <span class="ml-2" style="color: #a87000; font-weight: 600;">{{ dailyProgram.backwardness }}</span>
                                </div>
                                <div>
                                    <span style="color: #6a8090;">Adelantos:</span>
                                    <span class="ml-2" style="color: #0a7c3e; font-weight: 600;">{{ dailyProgram.advanced }}</span>
                                </div>
                                <div>
                                    <span style="color: #6a8090;">Total Fabricado:</span>
                                    <span class="ml-2" style="color: #0c1c28; font-weight: 600;">{{ dailyProgram.total_produced || 0 }}</span>
                                </div>
                                <div>
                                    <span style="color: #6a8090;">Total Rechazado:</span>
                                    <span class="ml-2" style="color: #ba2418; font-weight: 600;">{{ dailyProgram.total_rejected || 0 }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div v-else-if="selectedProgramId && selectedWorkCenterId && selectedPhase" class="text-center py-8">
                    <p style="color: #6a8090; font-weight: 600;">No hay programas diarios para la fecha y centro seleccionados</p>
                </div>

                <!-- Modal de Edición -->
                <div v-if="editingProgram" class="fixed inset-0 flex items-center justify-center z-50" style="background: rgba(11, 28, 40, 0.5);">
                    <div class="rounded-lg p-6 w-full max-w-2xl mx-4" style="background: #fff; border: 1px solid #d4dee8; box-shadow: 0 12px 40px rgba(0,0,0,.3);">
                        <h2 class="text-xl font-semibold mb-4" style="color: #0b2a40;">Editar Programa Diario</h2>
