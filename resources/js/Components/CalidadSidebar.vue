            ...
            >
                <div class="flex flex-col h-full">
                    <div class="p-6 border-b border-gray-700">
                        <h2 class="text-xl font-bold text-white">Calidad</h2>
                        <p class="text-sm text-gray-400 mt-1">Control de Calidad</p>
                    </div>

                    <nav class="flex-1 px-4 py-6 space-y-2">
                        <Link 
                            :href="route('calidad.registrar-rechazo')" 
                            class="sidebar-link"
                            :class="{ 'active': route().current('calidad.registrar-rechazo') }"
                        >
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                            </svg>
                            <span>Registrar Rechazo</span>
                        </Link>

                        <Link 
                            :href="route('calidad.rechazos')" 
                            class="sidebar-link"
                            :class="{ 'active': route().current('calidad.rechazos') }"
                        >
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
